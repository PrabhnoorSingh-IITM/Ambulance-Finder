...
import { getNearbyHospitals } from '../services/firebase';
...
function App() {
    ...
    const fetchNearbyHospitals = async (location) => {
        try {
            const result = await getNearbyHospitals(location.latitude, location.longitude);
            setHospitals(result);
        } catch (error) {
    ...
    const updateAvailability = async (hospitalId, newStatus) => {
        try {
            const { updateHospitalAvailability } = await import('../services/firebase');
            await updateHospitalAvailability(hospitalId, newStatus);
